...
const navigationGroups = [
  ...
  { number: "03", title: "NEESBOT", items: [["01", "EVENTOS DO CHAT"], ["02", "QUOTES"], ["03", "AUTOMAÇÕES"], ["04", "ENCURTADOR DE LINKS"]] },
...
export default function ControlPage() {
  ...
  const createShortLink = async (event:FormEvent<HTMLFormElement>) => {
    ...
    try{new URL(shortLinkDraft.destinationUrl);}catch{setShortLinkState("error");return;}
    const next=[{id:crypto.randomUUID(),slug,label:shortLinkDraft.label.trim()||slug,destinationUrl:shortLinkDraft.destinationUrl.trim(),active:true,createdAt:new Date().toISOString()},...shortLinks];
    ...
    <aside className="control-sidebar">
      ...
      <nav aria-label="Áreas administrativas">
        {visibleGroups.map((group) => <section className={selectedArea.startsWith(`${group.number}-`) ? "active" : ""} key={group.number}>
          <button className="control-nav-category" type="button" onClick={() => setSelectedArea(`${group.number}-${group.items[0][0]}`)}><span>{group.number}</span><b>{group.title}</b><i>{selectedArea.startsWith(`${group.number}-`) ? "−" : "+"}</i></button>
          <div className="control-nav-children">{group.items.map(([number, title]) => <button className={selectedArea === `${group.number}-${number}` ? "selected" : ""} type="button" onClick={() => setSelectedArea(`${group.number}-${number}`)} key={`${group.number}-${number}`}><span>{number}</span>{title}</button>)}</div>
        </section>)}
      </nav>
