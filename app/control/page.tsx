...
export default function ControlPage() {
  ...
  const saveBotCore = async (event:FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const normalizedCommands=botCommands.map((item)=>item.command.trim().toLowerCase());
    const invalidCommand=botCommands.some((item)=>!/^![a-z0-9_]+$/.test(item.command)||!item.response_template.trim())||new Set(normalizedCommands).size!==normalizedCommands.length;
    if(invalidCommand){setBotCommandError("REVISE OS COMANDOS: USE !NOME, EVITE DUPLICADOS E PREENCHA TODAS AS RESPOSTAS.");setBotSaveState("error");return;}
